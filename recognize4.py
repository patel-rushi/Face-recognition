...
for i, d in enumerate(dets):
    ...
    if conf > 30:
        ...
        if profile != None:
            cv2.putText(img,profile[1] + str("(%.2f)" % conf),(d.left(), d.bottom()),cv2.FONT_HERSHEY_SIMPLEX,2,(0, 0, 0),thickness=2)                                      # Writing the name of the face recognized
    else :
        cv2.putText(img,
                        "Unknown" + str(conf),
                        (d.left(), d.bottom()),cv2.FONT_HERSHEY_SIMPLEX,2,color=(0,255,0))                                                # Writing the name of the face recognized
